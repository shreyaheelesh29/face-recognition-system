...
# Video source (Change this path to the correct file location on your PC)
video_path = "us2.mp4"  # If the file is in the same directory
...
while True:
    ...
    if frame_count % frame_skip != 0:
        continue  # Skip every alternate frame (50% frame skipping)

    # Convert to RGB (face_recognition uses RGB format)
    rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

    # Detect faces
    face_locations = face_recognition.face_locations(rgb_frame, model='hog')  # Use 'cnn' if you have GPU
